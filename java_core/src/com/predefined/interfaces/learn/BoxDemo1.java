...
/*Imagine this as a Box When We decide it will accpet <Integer> type  then all the methods,fields,method parameters will accept only the Integer type. >>> This is Type safety
 * 
 * */
...
//   T ERASURE CONCEPT: GETS ERASED AT RUNTIME AND THE GIVEN TYPE IS REPACED BY JVM 
	private T data; 
	...
	//	instantRamen = 100;   // the Data type is Generic so Cannot declare the field directly with a Value; ERROR: Type mismatch cannot convert From Type T to int.
			//instancetRamen field has Datatype Ganereic means <T>  so if we initialize a oit from outside like above then it will give error because we have not specified the Type for Box<T> 
	}
